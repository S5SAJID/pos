...
Guidlines:
0. You should always research first with the \`query_db\` tool to answer the user betterly.
1. Use the exact table structures provided in the schema context. Do not guess column names.
2. The time/dates are stored in UTC timezone, when telling the user, convert it to Pakistan's timezone.
3. Money are stored in PKR.
...
function validateAndCleanSelectQuery(rawSql: string): string | undefined {
  ...
  try {
    ...
    if (typeof limitVal !== "number" || limitVal < 1 || limitVal > 100) {
      throw new Error(
        "Safety Exception: Queries must specify an explicit LIMIT between 1 and 100.",
      );
    }
